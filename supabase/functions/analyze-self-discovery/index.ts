...
6. 输出 3 个兴趣主题、3 个优势动作、3 个“用擅长的方式投入喜欢领域”的低成本验证方向。兴趣主题不要写成职业；优势动作不要写成知识领域。
...
function validateInput(value: unknown): DiscoveryInput {
  ...
  if (value.responses.length !== 71) {
    throw new HttpError(400, "INVALID_INPUT", "请完成全部 71 个探索问题。");
  }
  ...
  const responses = value.responses.map((item): DiscoveryResponse => {
    ...
    if (!isRecord(item.response)) {
      throw new HttpError(400, "INVALID_INPUT", "每个问题都需要完整回答。");
    }
    return {
      id,
      axis,
