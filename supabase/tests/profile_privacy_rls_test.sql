...
-- 最终画像模型：原子事实、提案审核、权限修订、公开发布边界与跨用户 RLS。
-- 运行：npm run db:test
...
begin
  ...
  v_permission_revision := public.replace_profile_ai_permissions(
    '{"skill":{"chat":true,"persona":false},"like":{"community":true}}'::jsonb,
    0
  );
  if v_permission_revision <> 1 or (
    select count(*) from public.profile_ai_permissions
  ) <> 3 then
    raise exception 'normalized permissions or permission revision are incorrect';
  end if;
  begin
    perform public.replace_profile_ai_permissions('{}'::jsonb, 0);
  ...
  perform public.save_public_profile(
    jsonb_build_object(
      ...
      'name', '公开主页保留',
      'bio', '仅公开低风险资料',
      'visibility', jsonb_build_object('skill', true, 'like', false)
    )
  );
  begin
    perform public.save_public_profile(
      '{"name":"bad","visibility":{"skill":"yes"}}'::jsonb
    );
    raise exception 'invalid public visibility unexpectedly succeeded';
  exception when invalid_parameter_value then null;
  end;
  if (
    select published_facts ? 'skill' and not (published_facts ? 'like')
      from public.public_profiles
     where id = '33333333-3333-4333-8333-333333333333'
  ) is not true then
    raise exception 'publication should include only explicitly visible dimensions';
  end if;

  select result.profile_revision
    into v_revision
    from public.delete_profile_dimension('skill', 3) result;
  if v_revision <> 4 then
    raise exception 'dimension delete should advance profile revision 3 -> 4, got %', v_revision;
  end if;
  if exists (
  ...
  select result.profile_revision
    into v_revision
    from public.clear_private_profile(5) result;
  if v_revision <> 6 then
    raise exception 'clear should advance profile revision 5 -> 6, got %', v_revision;
  end if;
  ...
  if (
    ...
    raise exception 'clear private profile must preserve public page and clear published facts';
  end if;
end
$$;
...
begin
  if (select count(*) from public.profile_facts) <> 0
     ...
     or (select count(*) from public.profile_public_drafts) <> 0
     or (select count(*) from public.profile_ai_permissions) <> 0 then
...
begin
  ...
  begin
    perform * from public.delete_profile_dimension('skill', 6);
