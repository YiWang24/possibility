...
struct SupabaseEventBackend: AnalyticsBackend {
    ...
    func track(_ event: AnalyticsEvent) {
        guard event.factDelivery == .all else { return }
        // 必须在同步调用点取时间；若放进 Task / actor 后再 Date()，突发事件会因调度延迟
        // 改变先后顺序，漏斗与留存的毫秒级时间线随之失真。
        let capturedAt = Date()
        Task { await buffer.enqueue(event, capturedAt: capturedAt) }
    }
...
private actor EventBuffer {
    ...
    func enqueue(_ event: AnalyticsEvent, capturedAt: Date) {
        ...
        pending.append(PendingEvent(
            row: EventRow(
                ...
                createdAt: timestampFormatter.string(from: capturedAt)
            )
        ))
        trimToCapacity()
