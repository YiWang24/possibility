...
final class ChatModel {
    ...
    func start(supabase: SupabaseService) async {
        ...
        if ExploreTopic.isSampleQuestion(launch.question) {
            ...
            // 本地示例没有服务端事实，必须由 iOS 写入 app_events。
            trackTurnCompleted(
                startedAt: startedAt,
    ...
    private func streamAssistant(
        ...
        // 真实 API 轮次由 chat Edge Function 权威写入 app_events；端上只保留
        // PostHog / Sentry breadcrumb，避免事实表双计数。
        trackTurnCompleted(
            startedAt: requestStartedAt,
            replyChars: messages[aiIndex].text.count,
            persistToFacts: false
        )
    ...
    private func trackTurnCompleted(
        startedAt: Date,
        replyChars: Int,
        persistToFacts: Bool
    ) {
        ...
        Analytics.shared.track(persistToFacts ? event : event.externalOnly())
    }
