      ...
      <div className="relative z-10 flex max-h-[80dvh] w-full flex-col screen-bg rounded-t-sheet md:max-w-[560px] md:rounded-sheet md:border md:border-line overflow-hidden">
        <div className="flex items-center gap-3 px-[22px] pt-5 pb-3">
          <div className="flex flex-col gap-0.5">
            <span className="text-subtitle font-semibold tracking-[0.8px] text-ink">历史探索</span>
            <span className="text-caption text-faint">选一段继续想下去</span>
          </div>
        </div>
            ...
            {entries.map((convo) => {
              ...
              return (
                <button
                  ...
                  className="text-left rounded-tile bg-card border border-line p-[15px] transition active:scale-[0.98]"
                >
                  <div className="flex items-center gap-2">
                    <Badge>{convo.topic}</Badge>
                    {ready ? <span className="text-micro tracking-[1px] text-teal">岔路口已成形</span> : null}
                    <div className="flex-1" />
                    {restoringId === convo.id ? (
                      <span className="w-3 h-3 rounded-full border-2 border-faint border-t-transparent animate-spin" />
                    ) : (
                      <span className="text-micro text-faint">{timeLabel(convo.created_at)}</span>
                    )}
                  </div>
                  <p className="text-body font-semibold leading-[1.5] text-ink mt-2.5 line-clamp-2">
