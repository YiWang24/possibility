...
export type DiscoveryAxis = "like" | "skill" | "energy" | "context" | "value";
...
export const DISCOVERY_QUESTIONS: DiscoveryQuestion[] = [
  ...
  q(
    ...
    "context",
    "适配环境 · 最好发挥",
    "在哪种工作或学习状态里，你最容易进入好状态？",
    "环境不决定能力，但会明显影响你能否稳定发挥。",
    [
      ["有自主空间，可以自己安排节奏", "自主空间", "✦"],
      ["和少数可靠的人紧密协作", "小团队共创", "♡"],
      ["目标、边界和标准都很清楚", "目标清晰", "▦"],
  ...
  q(
    ...
    "适配环境 · 容易消耗",
    "什么情况最容易让你的好状态被打断？",
    "识别边界不是挑剔，而是为了选择更可持续的投入方式。",
    [
      ["被过度控制、没有做法上的选择", "自主空间", "✦"],
      ["长期独自硬扛、缺少可信的讨论", "小团队共创", "♡"],
      ["目标反复变化、规则模糊", "目标清晰", "▦"],
      ["不断被碎片消息和临时任务打断", "连续深度", "◎"],
      ["做很久却不知道是否有用", "现实反馈", "◇"],
      ["长期重复、几乎没有新刺激", "多元变化", "↗"],
...
export function rankedWithCustom(
  axis: DiscoveryAxis,
  answers: Record<string, DiscoveryAnswer>,
): RankedTag[] {
