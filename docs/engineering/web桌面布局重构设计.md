...
1800×1100 实测（CDP `Emulation.setDeviceMetricsOverride` 精确视口，非 headless `--window-size`）：

| 页面 | 顶栏 logo 左沿 | 内容左沿 | 内容宽 | 利用率 | 页面级内滚 | 横向溢出 |
| --- | --- | --- | --- | --- | --- | --- |
| / | 188 | 188 | 1414 | 79% | 0 | 无 |
| /chat | 188 | 188 | 1414 | 79% | 0 | 无 |
| /community | 188 | 188 | 1414 | 79% | 0 | 无 |
| /me | 188 | 188 | 1414 | 79% | 0 | 无 |
| /diary | 188 | 188 | 1414 | 79% | 0 | 无 |
| /lab | 188 | 188 | 1414 | 79% | 0 | 无 |
| /studio | 188 | 188 | 1414 | 79% | 0 | 无 |

改造前后核心对比：

| 指标 | 前 | 后 |
| --- | --- | --- |
| /chat 内容宽 | 768（43%） | 1414（79%） |
| 首页内容宽 | 1062（59%） | 1414（79%） |
| 桌面内容左边界条数 | 4（292 / 369 / 193 / 516） | 1（188） |
| logo 与内容错位 | 78px | 0 |
| 页面级内部滚动条 | 13 | 0 |
| 首屏是否含数字形象 | 否（在折叠线以下） | 是 |

移动端 390×844：`scrollWidth == clientWidth == 390`，无横向溢出、无裁切，单列堆叠顺序与 iOS 一致。

顺带修掉一个**改造前就存在**的移动端缺陷：底部导航四个标签挤在 194px 内重叠。
两层原因叠加 —— NavigationMenu 根节点的 `max-w-max`，以及 Radix 在根节点与 `ul`
之间插入的无样式 `div` 作为 flex item 收缩到内容宽。修复后底栏满宽 370px，四格各 89px。

### 未纳入本次改造

`ProfileView`（旅人详情）、`BountyDetail`、`HubView`、`ResultView` 仍是「页头与正文
共用 measure 轴」的居中阅读式版式。它们内部自洽、无内部滚动条、无硬编码 `74px`，
按第 6 节的沉浸/阅读原型规则成立。其中 **旅人详情是漏斗的兑现页**
（「说清为什么推荐给你」），后续最值得加一条「为什么推荐给你 + 你的画像」侧轨。
...
9 个页面各自造了一条「圆形返回按钮 + 标题」的横条，其中 6 个带全宽 `border-b`、
3 个还叠了 `sticky` + `backdrop-blur` —— 视觉特征与真 navbar 完全一致，
紧贴全站导航之下就成了第二条。旅人详情最极端：返回横条 + hero 色带 + 吸顶 tab 栏，
三层类 navbar 结构叠在一起。

根因同样是 iOS idiom 的直译：app 没有浏览器后退键、也没有常驻导航，所以每屏必须自带返回。
web 两样都有，返回按钮能给的信息量反而更少 —— 它只能退一步，且不说明你身在第几层。

### 分型

页面裂成两种原型，各用各的解法：

**浏览型 → 面包屑**（GitHub / Linear / Notion / Vercel Dashboard 的做法）

`PageHeading` 组件承载「面包屑 + 标题 + 描述 + 右侧操作」。它不占独立横条：
没有 `border-b`、不 sticky、不铺满视口，与页面标题同属内容区、左对齐到全站中轴线。
上级由 `lib/nav.ts` 的 `sectionOf(pathname)` 从路由推导，命名直接复用顶栏 tab 名，
用户能把面包屑和导航对上号，也能一键跳回。

| 页面 | 面包屑 |
| --- | --- |
| /card-game | 认识你自己 › 卡牌探索 |
| /studio | 认识你自己 › 画像工坊 |
| /diary | 认识你自己 › 语音日记 |
| /chat | 认识你自己 › 探索 · 话题 |
| /traveler/[id] | 万花筒社区 › 旅人名 |
| /bounty/[id] | 万花筒社区 › 悬赏问题 |

规则收在组件里：**有面包屑就不渲染 eyebrow**。两者都在回答「这是哪儿」，
同时出现就是「认识你自己 › 卡牌探索 / LIFE CARDS / 卡牌探索」同一句话说三遍。

**任务型 → 专注模式**（Typeform / Google Forms / Stripe Checkout 的做法）

测评、卡牌局中、推演结果是有明确开始与结束的任务，本质是「模式」而不是「页面」。
`FocusShell` 收起全站导航，顶部只留一条极简条：左边任务名、中间进度、右边退出。
退出用 ✕ 而不是 ‹ —— 用户要的是「离开这个任务」，不是「回到上一屏」。
判定在 `lib/nav.ts` 的 `isFocusRoute`，只匹配带参数的子路由
（`/assessment/bigfive` 是任务，`/card-game` 是浏览型目录）；
lab 推演结果是 `/lab` 内的阶段态而非独立路由，用 `overlay` 显式声明。

弹层（对话总结）用右上角 ✕ 关闭而不是左侧 ‹：它是一层浮层，「返回」会让人以为退到了别的页面。

### 实测

| 页面 | 类 navbar 横条数 | 面包屑 |
| --- | --- | --- |
| /card-game · /studio · /diary · /chat · /traveler/1 · /bounty/1 | **1**（仅全站导航） | 正确 |
| /assessment/bigfive | **0**（全站导航已收起） | 不适用 |

全站 `BackButton` 调用点 9 → 0；两份重复的 `BackButton` 定义一并删除。
`lib/nav.ts` 成为路由结构的单一真源，顶栏、面包屑、专注模式判定共用一份。
